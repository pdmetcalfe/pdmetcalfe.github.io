...
#set document(title: [How fast can you count CpGs? (Rust edition)])
#metadata((
...
I got #link("https://xkcd.com/356/")[nerd-sniped] by
#link("https://sidderb.wordpress.com/2013/10/04/how-fast-can-you-count-the-number-of-cpgs-in-the-human-genome-2/")[Ben Sidders],
...
threading across the 24 chromosome files. That's a reasonable excuse to fiddle
with high-performance #link("https://rust-lang.org/")[Rust],
...
`str`, and `.lines()` checks every one of those bytes is
valid UTF-8 before it hands them back as an `&str`. Splitting on the
raw byte is more honest about what we're doing and, since we don't worry about 
the UTF-8 guarantee, strictly less work.)
...
takes about 190ms. Nothing clever, nothing wrong with it.#footnote[And also probably good enough for use in practice. But who wants "good enough" when we could have "perfection"?]
...
doesn't have to look at intrinsics directly:#footnote[I originally tried to be clever and write high-level code that would tempt LLVM's autovectorizer. Unfortunately I managed to tempt LLVM to pessimize the code by vectorizing along the wrong axis. The simplest way out was to do the intrinsics by hand, even if I'll have to write more code when I play on `x86_64`.]
...
and reduce with a *signed* widening sum (`vaddlvq_s8`), which adds up sixteen `-1`s and
...
This is *more* code. On chromosome 1 it took *~190ms* ---
identical to `simple`'s ~190ms. On the 17KB
mitochondrial file the two are close enough to call a tie (22.1µs vs
20.4µs). All that vectorized cleverness bought nothing,
...
bytes, newlines and all, which means every 61st (or so) byte pair (the `C` at the
...
I did of course run all four on every chromosome and check that they agree, mostly
so I'd notice immediately if I'd fouled-up#footnote[Other equivalent phrases are available.]
the NEON intrinsics. They do agree: 27,999,538 CpGs across everything.

The moral is of course profile before you optimize. The expensive part
of this problem was not the byte comparison --- it was a line-by-line read copying
253MB into a growing `Vec`. `mmap` fixed that in four lines and beat a hand-rolled NEON
kernel by 2.4x. SIMD earned its keep, but only *after* the actual bottleneck was gone;
applied to the wrong loop it's just more code that runs at the same speed as the code
it replaced.
