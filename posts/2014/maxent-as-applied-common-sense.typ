...
#set document(title: [Maximum entropy as applied common sense])
#metadata((
  title: "Maximum entropy as applied common sense",
  kind: "post",
  date: "2014-09-29",
  tags: ("maths", "statistics"),
  summary: "why roving tribes of statistical physicists might be useful",
)) <website-metadata>
...
$1 ... 6$ with mean $5$.  There's the obvious answer, of course,
...
of numerics (or grinding one's way through $tanh$).  The trick is
...
$
cal(L) = -sum_i p_i log p_i - mu(1 - sum_i p_i) - nu (5 - sum_i i p_i)
$

over $p_i$, $mu$, and $nu$.  ($mu$ and $nu$ are Lagrange
multipliers that enforce normalization and our mean constraint.)  We
find that

$
p_i = (exp i nu)/(sum_j exp j nu).
$

We've got to find $nu$ by enforcing the mean constraint, most
...
If our catnapper is a statistical physicist#footnote[Well, have you met any?] and is shouting about
ensembles and partition functions we can of course tell them that
$
(sum_i i e^(nu i))/(sum_i e^(nu i)) = S^(-1) (partial)/(partial nu) S = (partial)/(partial nu) log S,
$
where
$
S = sum_i e^(nu i) = (e^nu - e^(7 nu))/(1 - e^nu).
$